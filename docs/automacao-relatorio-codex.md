...
3. **`enviarWhatsApp`/`calcularLeadScore` são invocáveis por endpoint direto (Média).** Hoje só são chamadas internamente via `asServiceRole` (confirmado por grep), mas o endpoint não valida role/segredo. Verificar a política de invocação do Base44 e, se endpoints forem públicos, adicionar guard (role admin/interno ou segredo compartilhado) para evitar spam com custo (WhatsApp pago) e leitura de score.
