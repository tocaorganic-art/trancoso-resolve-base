...
export default function SejaPrestadorPage() {
  ...
  return (
    <div className="bg-background">
      ...
      {/* Benefícios */}
      <section className="bg-card border-y border-border py-10 md:py-20">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="text-center mb-6 md:mb-12">
            <span className="text-sm font-semibold text-primary uppercase tracking-widest">Vantagens exclusivas</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            {beneficios.map((item, index) => (
              <div key={index} className="group bg-card rounded-2xl p-6 md:p-8 border border-border hover:border-primary/50 shadow-lg hover:shadow-xl transition-all duration-300">
                <div className="w-14 h-14 md:w-16 md:h-16 bg-secondary rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-primary/10 transition-colors">
                  {item.icon}
                </div>
                <h3 className="text-lg md:text-xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors">{item.title}</h3>
      ...
      {/* CTA Final */}
      <div className="bg-card border-t border-border py-16 md:py-24">
        <div className="container mx-auto text-center px-4 max-w-2xl">
          <span className="inline-block bg-primary/10 border border-primary/30 text-primary text-xs font-semibold uppercase tracking-widest px-4 py-1.5 rounded-full mb-4">Vagas limitadas por categoria</span>
          <h2 className="text-3xl md:text-5xl font-bold text-foreground mb-4 leading-tight">
            Sua Expertise no<br />
            <span className="text-primary">Coração de Trancoso.</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to={createPageUrl('CadastroTipo')}>
              <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-lg px-10 py-4 shadow-xl min-h-[52px]">
                Cadastre-se Agora — É Grátis
              </Button>
            </Link>
            <Link to={createPageUrl('Planos')}>
              <Button size="lg" variant="outline" className="font-semibold px-8 min-h-[52px]">
                Ver Planos e Preços
              </Button>
            </Link>
          </div>
